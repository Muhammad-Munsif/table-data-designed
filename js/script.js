document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderTable(data = filteredData) {
    ...
    paginatedData.forEach((item) => {
      ...
      const highlightedPhone =
        searchTerm && item.phone
          ? highlightText(item.phone, searchTerm)
          : item.phone || "N/A";
      const highlightedDept = searchTerm
        ? highlightText(item.department, searchTerm)
        : item.department;

      tr.innerHTML = `
            <td data-label="ID"><span class="badge bg-light text-dark">${
              item.id
            }</span></td>
            <td data-label="Name">
            ...
            <td data-label="Email">
              <a href="mailto:${
                item.email
              }" class="text-decoration-none">${highlightedEmail}</a>
            </td>
            ...
            <td data-label="Join Date">
              <div class="d-flex flex-column">
                ...
                <small class="text-muted">${calculateTenure(
                  item.joinDate
                )}</small>
              </div>
            </td>
            <td data-label="Actions">
              <div class="btn-group-actions">
                ...
                <button class="btn btn-sm btn-delete delete-btn" data-id="${
                  item.id
                }" title="Delete">
                  <i class="fas fa-trash-alt"></i>
                </button>
                <button class="btn btn-sm btn-outline-primary view-btn" data-id="${
                  item.id
                }" title="View Details">
  ...
  function updateTableInfo(data = filteredData) {
    const startIndex =
      data.length > 0 ? (currentPage - 1) * rowsPerPage + 1 : 0;
  ...
  function updateSearchInfo() {
    ...
    if (searchTerm) {
      ...
      document
        ...
        ?.addEventListener("click", clearSearchHandler);
    } else {
      searchInfo.textContent =
        "Start typing to search employees by name, email, phone, or department";
    }
  }
  ...
  function handleEdit(e) {
    ...
    if (record) {
      document.getElementById(
        "modalTitle"
      ).innerHTML = `<i class="fas fa-user-edit me-2"></i>Edit Employee`;
  ...
  function handleView(e) {
    ...
    if (record) {
      const message = `
            ...
            <strong>Joined:</strong> ${formatDate(
              record.joinDate
            )} (${calculateTenure(record.joinDate)})<br>
  ...
  function handleAddNew() {
    document.getElementById(
      "modalTitle"
    ).innerHTML = `<i class="fas fa-user-plus me-2"></i>Add New Employee`;
    dataForm.reset();
  ...
  function saveData() {
    ...
    const duplicateEmail = tableData.find(
      (item) =>
